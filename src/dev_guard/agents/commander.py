...
class CommanderAgent(BaseAgent):
    ...
    async def execute(self, state: Any) -> Any:
        ...
        try:
            # Do not set a fake task id; keep current_task None for oversight
            self.set_status("busy")
            self.update_heartbeat()

            # Log system status
            # Log a JSON-serializable summary to avoid datetime serialization issues
            try:
                if hasattr(state, "model_dump"):
                    st = state
                    summary = {
                        "has_current_task": bool(getattr(st, "current_task", None)),
                        "pending_tasks": len(getattr(st, "pending_tasks", []) or []),
                        "active_agents": len(getattr(st, "active_agents", []) or []),
                        "last_update": getattr(st, "last_update", None).isoformat()
                        if getattr(st, "last_update", None)
                        else None,
                    }
                else:
                    summary = {"state_str": str(state)[:500]}
            except Exception:
                summary = {"state_str": str(state)[:200]}
    ...
    async def _check_agent_health(self) -> dict[str, Any]:
        ...
        health_report = {"healthy": [], "unhealthy": [], "unresponsive": []}
        ...
        self.log_observation(
            "Agent health check completed", data=health_report, tags=["health_check", "monitoring"]
        )
    ...
    async def _process_notifications(self) -> None:
        ...
        Always evaluate recent errors; only attempt to send notifications if a
        NotificationManager is configured.

        Order of operations ensures the last log_observation is about critical
        errors when present (per unit test expectation).
        """
        # First, check for system health issues (this may log observations)
        health_report = await self._check_agent_health()
        if (
            health_report.get("unresponsive") or health_report.get("unhealthy")
        ) and self.notification_manager:
            await self._send_health_alert_notifications(health_report)

        # Then, evaluate recent error entries and log critical errors last
        recent_errors = self.get_recent_memories(
            memory_type="error", limit=10, include_other_agents=True
        )

        critical_errors = [
            error
            for error in recent_errors
            if "critical" in (error.tags or set()) or "swarm_failure" in (error.tags or set())
        ]
        ...
        if critical_errors:
            ...
            # Send critical error notifications if manager available
        ...
        try:
            for error in errors[:3]:  # Limit to prevent spam
                context = {
                    ...
                    "timestamp": getattr(error, "timestamp", "Unknown").strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                    if hasattr(getattr(error, "timestamp", None), "strftime")
                    else str(getattr(error, "timestamp", "Unknown")),
                }
    ...
    async def _evaluate_system_status(self) -> dict[str, Any]:
        ...
        success_rate = (
            task_stats["completed"] / completed_or_failed if completed_or_failed > 0 else 1.0
        )
        ...
        self.log_observation(
            "System status evaluation completed", data=system_status, tags=["system_status"]
        )
    ...
    async def _categorize_request(self, request: str) -> str:
        ...
        if any(
            word in request_lower for word in ["code", "implement", "function", "class", "method"]
        ):
            return "code_generation"
        elif any(word in request_lower for word in ["document", "docs", "readme", "comment"]):
            return "documentation"
        elif any(
            word in request_lower for word in ["analyze", "impact", "dependenc", "relationship"]
        ):
            return "analysis"
